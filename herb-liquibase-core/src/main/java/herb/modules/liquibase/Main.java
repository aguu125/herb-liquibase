...
/**
 * 无法解决Main在springboot jar 运行模式下，所以直接拷贝重写，注释掉liquibase.properties 的代码
 * 反正我也用不到他，都是自定义配置文件动态加载的
 * parseOptions -> parseDefaultPropertyFiles -> parseDefaultPropertyFileFromResource
...
public class Main {
    ...
    public static int run(String[] args) throws LiquibaseException {
        ...
        try {
            ...
            if ((args.length == 0) || ((args.length == 1) && ("--" + Main.OPTIONS.HELP).equals(args[0]))) {
                main.printHelp(System.out);
                return 0;
            } else if (("--" + Main.OPTIONS.VERSION).equals(args[0])) {
                main.command = "";
                main.reconfigureLogging();
            ...
            for (int i = 0; i < args.length; i++) {
                Main.CodePointCheck codePointCheck = checkArg(args[i]);
            ...
            if (Main.COMMANDS.UPDATE.equals(main.command)) {
                log.info(LogType.USER_MESSAGE, coreBundle.getString("update.successful"));
            } else if (main.command.startsWith(Main.COMMANDS.ROLLBACK) && !main.command.endsWith("SQL")) {
    ...
    protected void reconfigureLogging() {
        ...
        if (this.logFile != null) {
            ...
            Main.ConsoleFilter logOutputFilter = new Main.ConsoleFilter(fileLogLevel);
        ...
        if (logLevel == Level.OFF) {
            Main.ConsoleFilter userOutput = new Main.ConsoleFilter(logLevel, LogType.USER_MESSAGE);
            ...
            Main.ConsoleFilter userOutput = new Main.ConsoleFilter(logLevel);
            userOutput.setContext(rootLogger.getLoggerContext());
            userOutput.start();
    ...
    private static boolean isStandardOutputRequired(String command) {
        return Main.COMMANDS.SNAPSHOT.equalsIgnoreCase(command)
                || Main.COMMANDS.SNAPSHOT_REFERENCE.equalsIgnoreCase(command)
                || Main.COMMANDS.CHANGELOG_SYNC_SQL.equalsIgnoreCase(command)
                || Main.COMMANDS.MARK_NEXT_CHANGESET_RAN_SQL.equalsIgnoreCase(command)
                || Main.COMMANDS.UPDATE_COUNT_SQL.equalsIgnoreCase(command)
                || Main.COMMANDS.UPDATE_TO_TAG_SQL.equalsIgnoreCase(command)
                || Main.COMMANDS.UPDATE_SQL.equalsIgnoreCase(command)
                || Main.COMMANDS.ROLLBACK_SQL.equalsIgnoreCase(command)
                || Main.COMMANDS.ROLLBACK_TO_DATE_SQL.equalsIgnoreCase(command)
                || Main.COMMANDS.ROLLBACK_COUNT_SQL.equalsIgnoreCase(command)
                || Main.COMMANDS.FUTURE_ROLLBACK_SQL.equalsIgnoreCase(command)
                || Main.COMMANDS.FUTURE_ROLLBACK_COUNT_SQL.equalsIgnoreCase(command)
                || Main.COMMANDS.FUTURE_ROLLBACK_FROM_TAG_SQL.equalsIgnoreCase(command);
    }
    ...
    private static boolean isChangeLogRequired(String command) {
        return command.toLowerCase().startsWith(Main.COMMANDS.UPDATE)
                || (command.toLowerCase().startsWith(Main.COMMANDS.ROLLBACK) &&
                (!command.equalsIgnoreCase(Main.COMMANDS.ROLLBACK_ONE_CHANGE_SET) &&
                        !command.equalsIgnoreCase(Main.COMMANDS.ROLLBACK_ONE_UPDATE)))
                || Main.COMMANDS.CALCULATE_CHECKSUM.equalsIgnoreCase(command)
                || Main.COMMANDS.STATUS.equalsIgnoreCase(command)
                || Main.COMMANDS.VALIDATE.equalsIgnoreCase(command)
                || Main.COMMANDS.CHANGELOG_SYNC.equalsIgnoreCase(command)
                || Main.COMMANDS.CHANGELOG_SYNC_SQL.equalsIgnoreCase(command)
                || Main.COMMANDS.GENERATE_CHANGELOG.equalsIgnoreCase(command)
                || Main.COMMANDS.DIFF_CHANGELOG.equalsIgnoreCase(command)
                || Main.COMMANDS.ROLLBACK_ONE_CHANGE_SET.equalsIgnoreCase(command)
                || Main.COMMANDS.ROLLBACK_ONE_UPDATE.equalsIgnoreCase(command);
    }
    ...
    private static boolean isCommand(String arg) {
        return Main.COMMANDS.MIGRATE.equals(arg)
                || Main.COMMANDS.MIGRATE_SQL.equalsIgnoreCase(arg)
                || Main.COMMANDS.UPDATE.equalsIgnoreCase(arg)
                || Main.COMMANDS.UPDATE_SQL.equalsIgnoreCase(arg)
                || Main.COMMANDS.UPDATE_COUNT.equalsIgnoreCase(arg)
                || Main.COMMANDS.UPDATE_COUNT_SQL.equalsIgnoreCase(arg)
                || Main.COMMANDS.UPDATE_TO_TAG.equalsIgnoreCase(arg)
                || Main.COMMANDS.UPDATE_TO_TAG_SQL.equalsIgnoreCase(arg)
                || Main.COMMANDS.ROLLBACK.equalsIgnoreCase(arg)
                || Main.COMMANDS.ROLLBACK_TO_DATE.equalsIgnoreCase(arg)
                || Main.COMMANDS.ROLLBACK_COUNT.equalsIgnoreCase(arg)
                || Main.COMMANDS.ROLLBACK_SQL.equalsIgnoreCase(arg)
                || Main.COMMANDS.ROLLBACK_TO_DATE_SQL.equalsIgnoreCase(arg)
                || Main.COMMANDS.ROLLBACK_COUNT_SQL.equalsIgnoreCase(arg)
                || Main.COMMANDS.FUTURE_ROLLBACK_SQL.equalsIgnoreCase(arg)
                || Main.COMMANDS.FUTURE_ROLLBACK_COUNT_SQL.equalsIgnoreCase(arg)
                || Main.COMMANDS.FUTURE_ROLLBACK_TO_TAG_SQL.equalsIgnoreCase(arg)
                || Main.COMMANDS.UPDATE_TESTING_ROLLBACK.equalsIgnoreCase(arg)
                || Main.COMMANDS.TAG.equalsIgnoreCase(arg)
                || Main.COMMANDS.TAG_EXISTS.equalsIgnoreCase(arg)
                || Main.COMMANDS.LIST_LOCKS.equalsIgnoreCase(arg)
                || Main.COMMANDS.HISTORY.equalsIgnoreCase(arg)
                || Main.COMMANDS.DROP_ALL.equalsIgnoreCase(arg)
                || Main.COMMANDS.RELEASE_LOCKS.equalsIgnoreCase(arg)
                || Main.COMMANDS.STATUS.equalsIgnoreCase(arg)
                || Main.COMMANDS.UNEXPECTED_CHANGESETS.equalsIgnoreCase(arg)
                || Main.COMMANDS.VALIDATE.equalsIgnoreCase(arg)
                || Main.COMMANDS.HELP.equalsIgnoreCase(arg)
                || Main.COMMANDS.DIFF.equalsIgnoreCase(arg)
                || Main.COMMANDS.DIFF_CHANGELOG.equalsIgnoreCase(arg)
                || Main.COMMANDS.GENERATE_CHANGELOG.equalsIgnoreCase(arg)
                || Main.COMMANDS.SNAPSHOT.equalsIgnoreCase(arg)
                || Main.COMMANDS.SNAPSHOT_REFERENCE.equalsIgnoreCase(arg)
                || Main.COMMANDS.EXECUTE_SQL.equalsIgnoreCase(arg)
                || Main.COMMANDS.CALCULATE_CHECKSUM.equalsIgnoreCase(arg)
                || Main.COMMANDS.CLEAR_CHECKSUMS.equalsIgnoreCase(arg)
                || Main.COMMANDS.DB_DOC.equalsIgnoreCase(arg)
                || Main.COMMANDS.CHANGELOG_SYNC.equalsIgnoreCase(arg)
                || Main.COMMANDS.CHANGELOG_SYNC_SQL.equalsIgnoreCase(arg)
                || Main.COMMANDS.MARK_NEXT_CHANGESET_RAN.equalsIgnoreCase(arg)
                || Main.COMMANDS.MARK_NEXT_CHANGESET_RAN_SQL.equalsIgnoreCase(arg)
                || Main.COMMANDS.ROLLBACK_ONE_CHANGE_SET.equalsIgnoreCase(arg)
                || Main.COMMANDS.ROLLBACK_ONE_CHANGE_SET_SQL.equalsIgnoreCase(arg)
                || Main.COMMANDS.ROLLBACK_ONE_UPDATE.equalsIgnoreCase(arg)
                || Main.COMMANDS.ROLLBACK_ONE_UPDATE_SQL.equalsIgnoreCase(arg);
    }
    ...
    protected List<String> checkSetup() {
        ...
        if (command == null) {
            ...
            if (StringUtils.trimToNull(url) == null) {
                messages.add(String.format(coreBundle.getString("option.required"), "--" + Main.OPTIONS.URL));
            }

            if (isChangeLogRequired(command) && (StringUtils.trimToNull(changeLogFile) == null)) {
                messages.add(String.format(coreBundle.getString("option.required"), "--" + Main.OPTIONS.CHANGELOG_FILE));
            }
    ...
    private void checkForUnexpectedCommandParameter(List<String> messages) {
        if (Main.COMMANDS.UPDATE_COUNT.equalsIgnoreCase(command)
                || Main.COMMANDS.UPDATE_COUNT_SQL.equalsIgnoreCase(command)
                || Main.COMMANDS.UPDATE_TO_TAG.equalsIgnoreCase(command)
                || Main.COMMANDS.UPDATE_TO_TAG_SQL.equalsIgnoreCase(command)
                || Main.COMMANDS.CALCULATE_CHECKSUM.equalsIgnoreCase(command)
                || Main.COMMANDS.DB_DOC.equalsIgnoreCase(command)
                || Main.COMMANDS.TAG.equalsIgnoreCase(command)
                || Main.COMMANDS.TAG_EXISTS.equalsIgnoreCase(command)) {

            if ((!commandParams.isEmpty()) && commandParams.iterator().next().startsWith("-")) {
                messages.add(coreBundle.getString(ERRORMSG_UNEXPECTED_PARAMETERS) + commandParams);
            }
        } else if (Main.COMMANDS.STATUS.equalsIgnoreCase(command)
                || Main.COMMANDS.UNEXPECTED_CHANGESETS.equalsIgnoreCase(command)) {
            if ((!commandParams.isEmpty())
                    && !commandParams.iterator().next().equalsIgnoreCase("--" + Main.OPTIONS.VERBOSE)) {
                messages.add(coreBundle.getString(ERRORMSG_UNEXPECTED_PARAMETERS) + commandParams);
            }
        } else if (Main.COMMANDS.DIFF.equalsIgnoreCase(command)
                || Main.COMMANDS.DIFF_CHANGELOG.equalsIgnoreCase(command)) {
            if ((!commandParams.isEmpty())) {
                for (String cmdParm : commandParams) {
                    if (!cmdParm.startsWith("--" + Main.OPTIONS.REFERENCE_USERNAME)
                            && !cmdParm.startsWith("--" + Main.OPTIONS.REFERENCE_PASSWORD)
                            && !cmdParm.startsWith("--" + Main.OPTIONS.REFERENCE_DRIVER)
                            && !cmdParm.startsWith("--" + Main.OPTIONS.REFERENCE_DEFAULT_CATALOG_NAME)
                            && !cmdParm.startsWith("--" + Main.OPTIONS.REFERENCE_DEFAULT_SCHEMA_NAME)
                            && !cmdParm.startsWith("--" + Main.OPTIONS.INCLUDE_SCHEMA)
                            && !cmdParm.startsWith("--" + Main.OPTIONS.INCLUDE_CATALOG)
                            && !cmdParm.startsWith("--" + Main.OPTIONS.INCLUDE_TABLESPACE)
                            && !cmdParm.startsWith("--" + Main.OPTIONS.SCHEMAS)
                            && !cmdParm.startsWith("--" + Main.OPTIONS.OUTPUT_SCHEMAS_AS)
                            && !cmdParm.startsWith("--" + Main.OPTIONS.REFERENCE_SCHEMAS)
                            && !cmdParm.startsWith("--" + Main.OPTIONS.REFERENCE_URL)
                            && !cmdParm.startsWith("--" + Main.OPTIONS.EXCLUDE_OBJECTS)
                            && !cmdParm.startsWith("--" + Main.OPTIONS.INCLUDE_OBJECTS)
                            && !cmdParm.startsWith("--" + Main.OPTIONS.DIFF_TYPES)

                            && !cmdParm.startsWith("--" + Main.OPTIONS.SNAPSHOT_FORMAT)) {
                        messages.add(String.format(coreBundle.getString("unexpected.command.parameter"), cmdParm));
                    }
                }
            }
        } else if ((Main.COMMANDS.SNAPSHOT.equalsIgnoreCase(command)
                || Main.COMMANDS.GENERATE_CHANGELOG.equalsIgnoreCase(command))
                && (!commandParams.isEmpty())) {
            for (String cmdParm : commandParams) {
                if (!cmdParm.startsWith("--" + Main.OPTIONS.INCLUDE_SCHEMA)
                        && !cmdParm.startsWith("--" + Main.OPTIONS.INCLUDE_CATALOG)
                        && !cmdParm.startsWith("--" + Main.OPTIONS.INCLUDE_TABLESPACE)
                        && !cmdParm.startsWith("--" + Main.OPTIONS.SCHEMAS)
                        && !cmdParm.startsWith("--" + Main.OPTIONS.SNAPSHOT_FORMAT)
                        && !cmdParm.startsWith("--" + Main.OPTIONS.DATA_OUTPUT_DIRECTORY)
                        && !cmdParm.startsWith("--" + Main.OPTIONS.OUTPUT_SCHEMAS_AS)) {
                    messages.add(String.format(coreBundle.getString("unexpected.command.parameter"), cmdParm));
                }
            }
        } else if (Main.COMMANDS.ROLLBACK_ONE_CHANGE_SET.equalsIgnoreCase(command)) {
            for (String cmdParm : commandParams) {
                if (!cmdParm.startsWith("--" + Main.OPTIONS.CHANGE_SET_ID)
                        && !cmdParm.startsWith("--" + Main.OPTIONS.HELP)
                        && !cmdParm.startsWith("--" + Main.OPTIONS.FORCE)
                        && !cmdParm.startsWith("--" + Main.OPTIONS.CHANGE_SET_PATH)
                        && !cmdParm.startsWith("--" + Main.OPTIONS.CHANGE_SET_AUTHOR)
                        && !cmdParm.startsWith("--" + Main.OPTIONS.ROLLBACK_SCRIPT)) {
                    messages.add(String.format(coreBundle.getString("unexpected.command.parameter"), cmdParm));
                }
            }
        } else if (Main.COMMANDS.ROLLBACK_ONE_CHANGE_SET_SQL.equalsIgnoreCase(command)) {
            for (String cmdParm : commandParams) {
                if (!cmdParm.startsWith("--" + Main.OPTIONS.CHANGE_SET_ID)
                        && !cmdParm.startsWith("--" + Main.OPTIONS.HELP)
                        && !cmdParm.startsWith("--" + Main.OPTIONS.FORCE)
        ...
        } else if (Main.COMMANDS.ROLLBACK_ONE_UPDATE.equalsIgnoreCase(command)) {
            for (String cmdParm : commandParams) {
                if (!cmdParm.startsWith("--" + Main.OPTIONS.DEPLOYMENT_ID)
                        && !cmdParm.startsWith("--" + Main.OPTIONS.HELP)
                        && !cmdParm.startsWith("--" + Main.OPTIONS.FORCE)) {
                    messages.add(String.format(coreBundle.getString("unexpected.command.parameter"), cmdParm));
                }
            }
        } else if (Main.COMMANDS.ROLLBACK_ONE_UPDATE_SQL.equalsIgnoreCase(command)) {
            for (String cmdParm : commandParams) {
                if (!cmdParm.startsWith("--" + Main.OPTIONS.DEPLOYMENT_ID)
                        && !cmdParm.startsWith("--" + Main.OPTIONS.HELP)
                        && !cmdParm.startsWith("--" + Main.OPTIONS.FORCE)) {
    ...
    private void checkForMalformedCommandParameters(final List<String> messages) {
        ...
        if (Main.COMMANDS.CALCULATE_CHECKSUM.equalsIgnoreCase(command)) {
        ...
        } else if (Main.COMMANDS.DIFF_CHANGELOG.equalsIgnoreCase(command) && (diffTypes != null) && diffTypes.toLowerCase
                ().contains("data")) {
            messages.add(String.format(coreBundle.getString("including.data.diffchangelog.has.no.effect"),
                    Main.OPTIONS.DIFF_TYPES, Main.COMMANDS.GENERATE_CHANGELOG
            ));
        }
    }
        ...
        for (int i = 0; i < chars.length; i++) {
            for (int j = 0; j < suspiciousCodePoints.length; j++) {
                if (suspiciousCodePoints[j] == chars[i]) {
                    Main.CodePointCheck codePointCheck = new Main.CodePointCheck();
    ...
    protected void parseOptions(String[] paramArgs) throws CommandLineParsingException {
        ...
        for (String arg : args) {
            ...
            if (isCommand(arg)) {
                this.command = arg;
                if (this.command.equalsIgnoreCase(Main.COMMANDS.MIGRATE)) {
                    this.command = Main.COMMANDS.UPDATE;
                } else if (this.command.equalsIgnoreCase(Main.COMMANDS.MIGRATE_SQL)) {
                    this.command = Main.COMMANDS.UPDATE_SQL;
                }
                seenCommand = true;
            } else if (seenCommand) {
                // ChangeLog parameter:
...
//        parseDefaultPropertyFiles();
    ...
    private void parseOptionArgument(String arg, boolean okIfNotAField) throws CommandLineParsingException {
        ...
        if (arg.toLowerCase().startsWith("--" + Main.OPTIONS.VERBOSE)) {
            return;
        }

        if (arg.toLowerCase().equals("--" + Main.OPTIONS.FORCE) || arg.toLowerCase().equals("--" + Main.OPTIONS.HELP)) {
            arg = arg + "=true";
        }
    ...
    protected void configureClassLoader() throws CommandLineParsingException {
        ...
        if (this.classpath != null) {
            ...
            for (String classpathEntry : classpathSoFar) {
                ...
                if (classpathEntry.endsWith(Main.FILE_SUFFIXES.WAR_FILE_SUFFIX)) {
                    try {
                        addWarFileClasspathEntries(classPathFile, urls);
                    } catch (IOException e) {
                        throw new CommandLineParsingException(e);
                    }
                } else if (classpathEntry.endsWith(Main.FILE_SUFFIXES.FILE_SUFFIX_EAR)) {
    ...
    protected void doMigration() throws Exception {
        if (Main.COMMANDS.HELP.equalsIgnoreCase(command)) {
        ...
        //
        if (Main.COMMANDS.ROLLBACK_ONE_CHANGE_SET.equals(command) ||
                Main.COMMANDS.ROLLBACK_ONE_CHANGE_SET_SQL.equals(command) ||
                Main.COMMANDS.ROLLBACK_ONE_UPDATE.equals(command) ||
                Main.COMMANDS.ROLLBACK_ONE_UPDATE_SQL.equals(command)) {
...
//            }
        ...
        try {
            if ((excludeObjects != null) && (includeObjects != null)) {
                throw new UnexpectedLiquibaseException(
                        String.format(coreBundle.getString("cannot.specify.both"),
                                Main.OPTIONS.EXCLUDE_OBJECTS, Main.OPTIONS.INCLUDE_OBJECTS));
            }
            ...
            if (Main.COMMANDS.DIFF.equalsIgnoreCase(command)) {
                ...
                return;
            } else if (Main.COMMANDS.DIFF_CHANGELOG.equalsIgnoreCase(command)) {
                ...
                return;
            } else if (Main.COMMANDS.GENERATE_CHANGELOG.equalsIgnoreCase(command)) {
                ...
                SnapshotCommand snapshotCommand = (SnapshotCommand) CommandFactory.getInstance()
                        .getCommand(Main.COMMANDS.SNAPSHOT);
                snapshotCommand.setDatabase(database);
                snapshotCommand.setSchemas(getSchemaParams(database));
                snapshotCommand.setSerializerFormat(getCommandParam(Main.OPTIONS.SNAPSHOT_FORMAT, null));
                ...
                outputWriter.close();
                return;
            } else if (Main.COMMANDS.SNAPSHOT_REFERENCE.equalsIgnoreCase(command)) {
                SnapshotCommand snapshotCommand = (SnapshotCommand) CommandFactory.getInstance()
                        .getCommand(Main.COMMANDS.SNAPSHOT);
            ...
            if (Main.COMMANDS.LIST_LOCKS.equalsIgnoreCase(command)) {
                liquibase.reportLocks(System.err);
                return;
            } else if (Main.COMMANDS.RELEASE_LOCKS.equalsIgnoreCase(command)) {
                ...
                return;
            } else if (Main.COMMANDS.TAG.equalsIgnoreCase(command)) {
                ...
                return;
            } else if (Main.COMMANDS.TAG_EXISTS.equalsIgnoreCase(command)) {
                ...
                return;
            } else if (Main.COMMANDS.ROLLBACK_ONE_CHANGE_SET.equals(command)) {
                Map<String, Object> argsMap = new HashMap<String, Object>();
                argsMap.put("changeSetId", getCommandParam(Main.OPTIONS.CHANGE_SET_ID, null));
                argsMap.put("changeSetAuthor", getCommandParam(Main.OPTIONS.CHANGE_SET_AUTHOR, null));
                argsMap.put("changeSetPath", getCommandParam(Main.OPTIONS.CHANGE_SET_PATH, null));
                ...
                LiquibaseCommand liquibaseCommand = createLiquibaseCommand(database, liquibase, Main.COMMANDS.ROLLBACK_ONE_CHANGE_SET, argsMap);
                liquibaseCommand.execute();
                return;
            } else if (Main.COMMANDS.ROLLBACK_ONE_CHANGE_SET_SQL.equals(command)) {
                Writer outputWriter = getOutputWriter();
                Map<String, Object> argsMap = new HashMap<String, Object>();
                ...
                outputWriter.close();
                return;
            } else if (Main.COMMANDS.ROLLBACK_ONE_UPDATE.equals(command)) {
                Map<String, Object> argsMap = new HashMap<String, Object>();
                argsMap.put("deploymentId", getCommandParam(Main.OPTIONS.DEPLOYMENT_ID, null));
                argsMap.put("changeLogFile", changeLogFile);
                argsMap.put("database", database);
                if (!commandParams.contains("--help")) {
                    argsMap.put("changeLog", liquibase.getDatabaseChangeLog());
                ...
                LiquibaseCommand liquibaseCommand = createLiquibaseCommand(database, liquibase, Main.COMMANDS.ROLLBACK_ONE_UPDATE, argsMap);
                liquibaseCommand.execute();
                return;
            } else if (Main.COMMANDS.ROLLBACK_ONE_UPDATE_SQL.equals(command)) {
                Writer outputWriter = getOutputWriter();
                Map<String, Object> argsMap = new HashMap<String, Object>();
                argsMap.put("deploymentId", getCommandParam(Main.OPTIONS.DEPLOYMENT_ID, null));
                argsMap.put("outputWriter", outputWriter);
                ...
                if (commandParams.contains("--" + Main.OPTIONS.VERBOSE)) {
                    runVerbose = true;
                }
                liquibase.reportStatus(runVerbose, new Contexts(contexts), new LabelExpression(labels),
                        getOutputWriter());
                return;
            } else if (Main.COMMANDS.UNEXPECTED_CHANGESETS.equalsIgnoreCase(command)) {
                boolean runVerbose = false;
                ...
                return;
            } else if (Main.COMMANDS.VALIDATE.equalsIgnoreCase(command)) {
                try {
                    liquibase.validate();
                ...
                return;
            } else if (Main.COMMANDS.CLEAR_CHECKSUMS.equalsIgnoreCase(command)) {
                liquibase.clearCheckSums();
                return;
            } else if (Main.COMMANDS.CALCULATE_CHECKSUM.equalsIgnoreCase(command)) {
                ...
                return;
            } else if (Main.COMMANDS.DB_DOC.equalsIgnoreCase(command)) {
                if (commandParams.isEmpty()) {
            ...
            try {
                if (Main.COMMANDS.UPDATE.equalsIgnoreCase(command)) {
                    liquibase.update(new Contexts(contexts), new LabelExpression(labels));
                } else if (Main.COMMANDS.CHANGELOG_SYNC.equalsIgnoreCase(command)) {
                    liquibase.changeLogSync(new Contexts(contexts), new LabelExpression(labels));
                } else if (Main.COMMANDS.CHANGELOG_SYNC_SQL.equalsIgnoreCase(command)) {
                    liquibase.changeLogSync(new Contexts(contexts), new LabelExpression(labels), getOutputWriter());
                } else if (Main.COMMANDS.MARK_NEXT_CHANGESET_RAN.equalsIgnoreCase(command)) {
                    liquibase.markNextChangeSetRan(new Contexts(contexts), new LabelExpression(labels));
                } else if (Main.COMMANDS.MARK_NEXT_CHANGESET_RAN_SQL.equalsIgnoreCase(command)) {
                    liquibase.markNextChangeSetRan(new Contexts(contexts), new LabelExpression(labels),
                            getOutputWriter());
                } else if (Main.COMMANDS.UPDATE_COUNT.equalsIgnoreCase(command)) {
                    liquibase.update(Integer.parseInt(commandParams.iterator().next()), new Contexts(contexts), new
                            LabelExpression(labels));
                } else if (Main.COMMANDS.UPDATE_COUNT_SQL.equalsIgnoreCase(command)) {
                    liquibase.update(Integer.parseInt(commandParams.iterator().next()), new Contexts(contexts), new
                            LabelExpression(labels), getOutputWriter());
                } else if (Main.COMMANDS.UPDATE_TO_TAG.equalsIgnoreCase(command)) {
                    if ((commandParams == null) || commandParams.isEmpty()) {
                        throw new CommandLineParsingException(
                                String.format(coreBundle.getString("command.requires.tag"), Main.COMMANDS.UPDATE_TO_TAG));
                    }

                    liquibase.update(commandParams.iterator().next(), new Contexts(contexts), new LabelExpression
                            (labels));
                } else if (Main.COMMANDS.UPDATE_TO_TAG_SQL.equalsIgnoreCase(command)) {
                    if ((commandParams == null) || commandParams.isEmpty()) {
                        throw new CommandLineParsingException(
                                String.format(coreBundle.getString("command.requires.tag"),
                                        Main.COMMANDS.UPDATE_TO_TAG_SQL));
                    }

                    liquibase.update(commandParams.iterator().next(), new Contexts(contexts), new LabelExpression
                            (labels), getOutputWriter());
                } else if (Main.COMMANDS.UPDATE_SQL.equalsIgnoreCase(command)) {
                    liquibase.update(new Contexts(contexts), new LabelExpression(labels), getOutputWriter());
                } else if (Main.COMMANDS.ROLLBACK.equalsIgnoreCase(command)) {
                    if (getCommandArgument() == null) {
                        throw new CommandLineParsingException(
                                String.format(coreBundle.getString("command.requires.tag"), Main.COMMANDS.ROLLBACK));
                    }
                    liquibase.rollback(getCommandArgument(), getCommandParam(Main.COMMANDS.ROLLBACK_SCRIPT, null), new
                            Contexts(contexts), new LabelExpression(labels));
                } else if (Main.COMMANDS.ROLLBACK_TO_DATE.equalsIgnoreCase(command)) {
                    if (getCommandArgument() == null) {
                        throw new CommandLineParsingException(
                                String.format(coreBundle.getString("command.requires.timestamp"),
                                        Main.COMMANDS.ROLLBACK_TO_DATE));
                    }
                    liquibase.rollback(new ISODateFormat().parse(getCommandArgument()), getCommandParam
                            (Main.COMMANDS.ROLLBACK_SCRIPT, null), new Contexts(contexts), new LabelExpression(labels));
                } else if (Main.COMMANDS.ROLLBACK_COUNT.equalsIgnoreCase(command)) {
                    liquibase.rollback(Integer.parseInt(getCommandArgument()), getCommandParam
                            (Main.COMMANDS.ROLLBACK_SCRIPT, null), new Contexts(contexts), new LabelExpression(labels));

                } else if (Main.COMMANDS.ROLLBACK_SQL.equalsIgnoreCase(command)) {
                    if (getCommandArgument() == null) {
                        throw new CommandLineParsingException(
                                String.format(coreBundle.getString("command.requires.tag"),
                                        Main.COMMANDS.ROLLBACK_SQL));
                    }
                    liquibase.rollback(getCommandArgument(), getCommandParam(Main.COMMANDS.ROLLBACK_SCRIPT, null), new
                            Contexts(contexts), new LabelExpression(labels), getOutputWriter());
                } else if (Main.COMMANDS.ROLLBACK_TO_DATE_SQL.equalsIgnoreCase(command)) {
                    if (getCommandArgument() == null) {
                        throw new CommandLineParsingException(
                                String.format(coreBundle.getString("command.requires.timestamp"),
                                        Main.COMMANDS.ROLLBACK_TO_DATE_SQL));
                    }
                    liquibase.rollback(new ISODateFormat().parse(getCommandArgument()), getCommandParam
                                    (Main.COMMANDS.ROLLBACK_SCRIPT, null), new Contexts(contexts), new LabelExpression
                                    (labels),
                            getOutputWriter());
                } else if (Main.COMMANDS.ROLLBACK_COUNT_SQL.equalsIgnoreCase(command)) {
                    if (getCommandArgument() == null) {
                        throw new CommandLineParsingException(
                                String.format(coreBundle.getString("command.requires.count"),
                                        Main.COMMANDS.ROLLBACK_COUNT_SQL));
                    }

                    liquibase.rollback(Integer.parseInt(getCommandArgument()), getCommandParam
                                    (Main.COMMANDS.ROLLBACK_SCRIPT, null), new Contexts(contexts),
                            new LabelExpression(labels),
                            getOutputWriter()
                    );
                } else if (Main.COMMANDS.FUTURE_ROLLBACK_SQL.equalsIgnoreCase(command)) {
                    liquibase.futureRollbackSQL(new Contexts(contexts), new LabelExpression(labels), getOutputWriter());
                } else if (Main.COMMANDS.FUTURE_ROLLBACK_COUNT_SQL.equalsIgnoreCase(command)) {
                    if (getCommandArgument() == null) {
                        throw new CommandLineParsingException(
                                String.format(coreBundle.getString("command.requires.count"),
                                        Main.COMMANDS.FUTURE_ROLLBACK_COUNT_SQL));
                    }

                    liquibase.futureRollbackSQL(Integer.parseInt(getCommandArgument()), new Contexts(contexts), new
                            LabelExpression(labels), getOutputWriter());
                } else if (Main.COMMANDS.FUTURE_ROLLBACK_FROM_TAG_SQL.equalsIgnoreCase(command)) {
                    if (getCommandArgument() == null) {
                        throw new CommandLineParsingException(
                                String.format(coreBundle.getString("command.requires.tag"),
                                        Main.COMMANDS.FUTURE_ROLLBACK_FROM_TAG_SQL));
                    }

                    liquibase.futureRollbackSQL(getCommandArgument(), new Contexts(contexts), new LabelExpression
                            (labels), getOutputWriter());
                } else if (Main.COMMANDS.UPDATE_TESTING_ROLLBACK.equalsIgnoreCase(command)) {
                    liquibase.updateTestingRollback(new Contexts(contexts), new LabelExpression(labels));
                } else if (Main.COMMANDS.HISTORY.equalsIgnoreCase(command)) {
    ...
    private String getSchemaParams(Database database) throws CommandLineParsingException {
        String schemaParams = getCommandParam(Main.OPTIONS.SCHEMAS, schemas);
    ...
    private Database createReferenceDatabaseFromCommandParams(
        ...
        for (String param : commandParams) {
            ...
            String attributeName = splitArg[0];
            String value = splitArg[1];
            if (Main.OPTIONS.REFERENCE_DRIVER.equalsIgnoreCase(attributeName)) {
                refDriver = value;
            } else if (Main.OPTIONS.REFERENCE_URL.equalsIgnoreCase(attributeName)) {
                refUrl = value;
            } else if (Main.OPTIONS.REFERENCE_USERNAME.equalsIgnoreCase(attributeName)) {
                refUsername = value;
            } else if (Main.OPTIONS.REFERENCE_PASSWORD.equalsIgnoreCase(attributeName)) {
                refPassword = value;
            } else if (Main.OPTIONS.REFERENCE_DEFAULT_CATALOG_NAME.equalsIgnoreCase(attributeName)) {
                defCatalogName = value;
            } else if (Main.OPTIONS.REFERENCE_DEFAULT_SCHEMA_NAME.equalsIgnoreCase(attributeName)) {
                defSchemaName = value;
            } else if (Main.OPTIONS.DATA_OUTPUT_DIRECTORY.equalsIgnoreCase(attributeName)) {
    ...
    private enum COMMANDS {
        ...
        private static final String HELP = Main.OPTIONS.HELP;
